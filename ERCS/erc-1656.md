...
This ERC establishes the first formal protocol standard for AI governance at the smart contract level, analogous to what [ERC-20](./erc-20.md) achieved for fungible tokens and [ERC-721](./erc-721.md) for non-fungible tokens.
...
Compliant contracts MUST implement [ERC-165](./erc-165.md) and return `true` for the following interface identifiers:
...
This ERC introduces a new standard and has no backwards compatibility issues with existing ERCs. Contracts implementing this ERC MAY coexist with [ERC-20](./erc-20.md) governance tokens, OpenZeppelin Governor contracts, and other existing governance infrastructure.
